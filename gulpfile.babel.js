...
// Create sass instance with compiler
...
gulp.task(
  ...
  gulp.series(
    clean,
    gulp.parallel(pages, javascript, images, copy, copyGallery),
    compileSass
  )
);
...
function copyGallery() {
  return gulp.src(PATHS.gallery).pipe(gulp.dest(PATHS.dist + "/gallery"));
}
...
function compileSass() {
  // Convert string patterns to regex patterns for UnCSS
  const uncssOptions = { ...UNCSS_OPTIONS };
  if (uncssOptions.ignore) {
    uncssOptions.ignore = uncssOptions.ignore.map((pattern) => {
  ...
  const postCssPlugins = [
    ...
    // PRODUCTION && uncss.postcssPlugin(uncssOptions),
  ].filter(Boolean);
...
function watch() {
  ...
  // gulp.watch(PATHS.gallery, copyGallery);
  gulp
